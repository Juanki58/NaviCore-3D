#!/usr/bin/env python3
"""
Batería de inyección de fallos — telemetría UDP NaviCore-3D (Unity unified).
"""

from __future__ import annotations

import socket
import sys
import unittest
from pathlib import Path

_REPO = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(_REPO / "tools" / "lib"))
sys.path.insert(0, str(Path(__file__).resolve().parent))
from navicore_paths import ensure_tools_path  # noqa: E402

ensure_tools_path(_REPO)

from telemetry_protocol import (  # noqa: E402
    UNITY_PACKET_SIZE,
    pack_unity_packet,
)
from telemetry_receiver import TelemetryReceiver  # noqa: E402


def _open_tx_rx() -> tuple[socket.socket, TelemetryReceiver, int]:
    rx = TelemetryReceiver(host="127.0.0.1", port=0, max_samples=200)
    port = rx.sock.getsockname()[1]
    tx = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    return tx, rx, port


def _pkt(
    timestamp_ms: int,
    pos_n_m: float = 0.0,
    pos_e_m: float = 0.0,
    pos_d_m: float = 0.0,
    vel_n_mps: float = 0.0,
    vel_e_mps: float = 0.0,
    vel_d_mps: float = 0.0,
    *,
    nav_mode: int = 1,
    mission_state: int = 0,
    health_mode: int = 0,
    flags: int = 0,
    score: int = 100,
    seq: int = 0,
    magic: int | None = None,
) -> bytes:
    kwargs = {}
    if magic is not None:
        kwargs["magic"] = magic
    return pack_unity_packet(
        timestamp_ms,
        pos_n_m,
        pos_e_m,
        pos_d_m,
        vel_n_mps,
        vel_e_mps,
        vel_d_mps,
        1.0,
        0.0,
        0.0,
        0.0,
        nav_mode,
        mission_state,
        health_mode,
        flags,
        score,
        seq=seq,
        **kwargs,
    )


class TestFaultInjection(unittest.TestCase):
    def test_reject_short_and_truncated_packets(self) -> None:
        tx, rx, port = _open_tx_rx()
        tx.sendto(b"\x00" * 16, ("127.0.0.1", port))
        tx.sendto(_pkt(0, 1.0, 2.0, 3.0)[:24], ("127.0.0.1", port))
        self.assertEqual(rx.drain(), 0)
        self.assertEqual(rx.packets_invalid, 2)
        tx.close()
        rx.close()

    def test_reject_truncated_and_oversized(self) -> None:
        tx, rx, port = _open_tx_rx()
        tx.sendto(b"\x00" * (UNITY_PACKET_SIZE - 1), ("127.0.0.1", port))
        tx.sendto(b"\x00" * (UNITY_PACKET_SIZE + 8), ("127.0.0.1", port))
        self.assertEqual(rx.drain(), 0)
        self.assertEqual(rx.packets_invalid, 2)
        tx.close()
        rx.close()

    def test_reject_bad_magic(self) -> None:
        tx, rx, port = _open_tx_rx()
        bad = _pkt(0, score=50, magic=0xDEAD)
        tx.sendto(bad, ("127.0.0.1", port))
        self.assertEqual(rx.drain(), 0)
        self.assertEqual(rx.packets_invalid, 1)
        tx.close()
        rx.close()

    def test_garbage_burst_then_valid(self) -> None:
        tx, rx, port = _open_tx_rx()
        for _ in range(20):
            tx.sendto(b"\xFF" * 8, ("127.0.0.1", port))
        good = _pkt(
            300,
            10.0,
            20.0,
            0.0,
            0.1,
            5.0,
            0.0,
            nav_mode=3,
            mission_state=3,
            health_mode=0,
            score=95,
            seq=1,
        )
        tx.sendto(good, ("127.0.0.1", port))
        self.assertEqual(rx.drain(), 1)
        self.assertEqual(rx.packets_invalid, 20)
        self.assertEqual(rx.samples[-1].mission_state_name, "NAVIGATE")
        self.assertEqual(rx.samples[-1].nav_mode_name, "HYBRID")
        tx.close()
        rx.close()

    def test_seq_gap_detection(self) -> None:
        tx, rx, port = _open_tx_rx()
        tx.sendto(_pkt(0, score=100, seq=10), ("127.0.0.1", port))
        tx.sendto(_pkt(100, 1.0, 1.0, score=90, seq=12), ("127.0.0.1", port))
        self.assertEqual(rx.drain(), 2)
        self.assertEqual(rx.seq_gaps, 1)
        tx.close()
        rx.close()

    def test_seq_wraparound_no_false_gap(self) -> None:
        tx, rx, port = _open_tx_rx()
        tx.sendto(_pkt(0, score=100, seq=0xFFFF), ("127.0.0.1", port))
        tx.sendto(_pkt(100, 1.0, score=95, seq=0), ("127.0.0.1", port))
        self.assertEqual(rx.drain(), 2)
        self.assertEqual(rx.seq_gaps, 0)
        tx.close()
        rx.close()

    def test_ring_buffer_caps_at_max_samples(self) -> None:
        tx, rx, port = _open_tx_rx()
        rx.close()
        tx.close()

        rx = TelemetryReceiver(host="127.0.0.1", port=0, max_samples=10)
        port = rx.sock.getsockname()[1]
        tx = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        for i in range(25):
            tx.sendto(
                _pkt(i * 100, float(i), 0.0, score=80, seq=i + 1),
                ("127.0.0.1", port),
            )
        self.assertEqual(rx.drain(), 25)
        self.assertEqual(len(rx.samples), 10)
        self.assertEqual(rx.samples[-1].x, 24.0)
        tx.close()
        rx.close()

    def test_hot_restart_marker_after_critical(self) -> None:
        tx, rx, port = _open_tx_rx()
        tx.sendto(_pkt(0, 1.0, 2.0, score=5, health_mode=2, seq=1), ("127.0.0.1", port))
        tx.sendto(_pkt(100, 1.5, 2.5, score=80, health_mode=0, seq=2), ("127.0.0.1", port))
        rx.drain()
        self.assertEqual(len(rx.recovery_points), 1)
        tx.close()
        rx.close()

    def test_critical_mode_survives_mixed_traffic(self) -> None:
        tx, rx, port = _open_tx_rx()
        tx.sendto(b"\x00" * 4, ("127.0.0.1", port))
        tx.sendto(
            _pkt(0, 5.0, 5.0, vel_n_mps=2.0, vel_e_mps=8.0, score=10, health_mode=2, nav_mode=2, seq=1),
            ("127.0.0.1", port),
        )
        self.assertEqual(rx.drain(), 1)
        sample = rx.samples[-1]
        self.assertEqual(sample.mode, "CRITICAL")
        self.assertAlmostEqual(sample.vel_n_mps, 2.0)
        self.assertAlmostEqual(sample.vel_e_mps, 8.0)
        tx.close()
        rx.close()


def main() -> int:
    print("=" * 60)
    print("NaviCore-3D — Batería de inyección de fallos UDP (Unity)")
    print("=" * 60)
    suite = unittest.defaultTestLoader.loadTestsFromModule(sys.modules[__name__])
    result = unittest.TextTestRunner(verbosity=2).run(suite)
    print("=" * 60)
    print(f"RESULTADO: {result.testsRun} pruebas — {'OK' if result.wasSuccessful() else 'FALLÓ'}")
    print("=" * 60)
    return 0 if result.wasSuccessful() else 1


if __name__ == "__main__":
    raise SystemExit(main())
