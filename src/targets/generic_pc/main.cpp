...
constexpr float kSuperTunnelSpeedMps = 25.0f; /* 90 km/h */
...
void super_tunnel_sanitize_imu(ImuSample *imu)
{
    ...
    /*
     * IMU ideal FRD alineada con sensors_sim / ins_ekf: en reposo accel ≈ [0, 0, +g].
     */
...
SuperTunnelPassResult super_tunnel_run_pass(bool nhc_enabled)
{
    ...
    for (uint32_t t_ms = 0U; t_ms <= kSuperTunnelDurationMs; t_ms += kEkfStepMs) {
        ...
        const bool gps_sample_fresh = gps.fix_valid;
        ...
        if (ekf_seeded && imu.valid) {
            ...
            if (t_ms == kSuperTunnelGpsOffStartMs) {
                float truth_n = 0.0f;
                float truth_e = 0.0f;
...
void run_super_tunnel_scenario(TelemetryInterface *telemetry)
{
    ...
    const SuperTunnelPassResult without_nhc = super_tunnel_run_pass(false);
    const SuperTunnelPassResult with_nhc = super_tunnel_run_pass(true);
